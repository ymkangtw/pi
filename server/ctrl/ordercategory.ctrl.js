...
async function getBy(req, res) {
	...
	const allowedKeys = Object.keys(db.ordercategory.rawAttributes);
	let queryobj = {};
	let conds = [];
...
async function create(req, res) {
	...
	try {
		await db.ordercategory.create(val, { transaction: t });
		await t.commit();
...
async function update(req, res) {
	...
	for(const key in val) {
		if (['categoryid'].indexOf(key) === -1) {
			updateobj[key] = val[key];
		}
	}
	const t = await db.sequelize.transaction();
	try {
		await db.ordercategory.update(updateobj, {
			where: {
...
async function remove(req, res) {
	...
	const t = await db.sequelize.transaction();
	try {
		await db.ordercategory.destroy({
			where: {
				categoryid: val.categoryid
			},
			transaction: t
