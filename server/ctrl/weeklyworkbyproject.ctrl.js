...
async function getAll(req, res) {
	let querystr = 
		...
		//"FORMAT(inputdate, 'yyyy-MM-dd') as inputdate, " +
		"convert(varchar(10), inputdate, 120) as inputdate, " +
		"content " +
		"from weeklyworkbyproject " +
		"order by jobno, subjobno, inputdate asc";
...
async function getBy(req, res) {
	...
	const allowedKeys = Object.keys(db.weeklyworkbyproject.rawAttributes);
	let queryobj = {};
	let conds = [];
	...
	let querystr = 
		...
		"convert(varchar(10), inputdate, 120) as inputdate, " +
		"content " +		
		"from weeklyworkbyproject " + wherestr +
		"order by jobno, subjobno, inputdate asc";
...
async function create(req, res) {
	...
	try {
		await db.weeklyworkbyproject.create(val, { transaction: t });
		await t.commit();
...
async function update(req, res) {
	...
	const t = await db.sequelize.transaction();
	try {
		await db.weeklyworkbyproject.update(updateobj, {
			where: {
				jobno: val.jobno,
...
async function remove(req, res) {
	...
	const t = await db.sequelize.transaction();
	try {
		await db.weeklyworkbyproject.destroy({
			where: {
				jobno: val.jobno,
				subjobno: val.subjobno,
				inputdate: val.inputdate
