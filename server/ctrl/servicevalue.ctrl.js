...
async function getAll(req, res) {
	let querystr = 
		"select item, " +
		"convert(varchar(10), setdate, 120) as setdate, " +
		"genvalue, dcsvalue, plcvalue, approvalvalue, comvalue, unithourcost " +
		"from servicevalue " + 
...
async function getBy(req, res) {
	...
	const allowedKeys = Object.keys(db.servicevalue.rawAttributes);
	let queryobj = {};
	let conds = [];
...
async function create(req, res) {
	...
	try {
		await db.servicevalue.create(val, { transaction: t });
		await t.commit();
...
async function update(req, res) {
	...
	const t = await db.sequelize.transaction();
	try {
		await db.servicevalue.update(updateobj, {
			where: {
				item: val.item
			},
			transaction: t
