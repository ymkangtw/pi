...
async function getBy(req, res) {
	...
	const allowedKeys = Object.keys(db.factorycode.rawAttributes);
	let queryobj = {};
	let conds = [];
...
async function create(req, res) {
	...
	try {
		await db.factorycode.create(val, { transaction: t });
		await t.commit();
...
async function update(req, res) {
	...
	const t = await db.sequelize.transaction();
	try {
		await db.factorycode.update(updateobj, {
			where: {
				fcode: val.fcode
			},
			transaction: t
