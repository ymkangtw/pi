...
- 承上，**同一類陷阱：`<style scoped>` 碰不到 Element Plus 元件的內部元素**，把版面樣式設在元件根元素上、指望影響內容物，等於搭在別人的實作細節上，對方一改就斷。已踩過一次：`PM02.vue` 的 `.option-card` 把 `display:flex; align-items:center; justify-content:center` 設在 `<el-card>` 根元素，2.11.5 時 `.el-card` 是 `display:block`、`.el-card__body` shrink-to-fit，於是**碰巧**置中；2.12 起 Element Plus 自己宣告 `.el-card{display:flex; flex-direction:column}` ＋ `.el-card__body{flex-grow:1}`，body 撐滿整張卡、卡片層的 `justify-content` 沒有剩餘空間可分配，垂直置中就失效（水平因為 `column` 模式下 `align-items` 管的是水平交叉軸而僥倖沒壞，症狀是「水平對、垂直偏上 13px」）。正解是用 `:deep(.el-card__body)` 讓 body 自己成為置中容器，不依賴卡片層；另注意 **Element Plus 沒有全域 `*{box-sizing:border-box}` reset**（逐元件宣告），要設 `width:100%` 加 padding 時得自己補 `box-sizing`。診斷手法：`getComputedStyle` 看 `flexDirection` 是否被對方蓋掉，再用 `Range.getBoundingClientRect()` 量文字上下左右間距（中文字型 glyph box 上下不對稱，殘留約 2.9px 誤差是正常的，要跟一個已知正確置中的參考容器對照才能判定）
- **`el-card` 不要寫死 `height`，用 `min-height`**：Element Plus 2.12 起 `.el-card__body` 加了 `overflow:auto`，內容一超出固定高度就冒捲軸。2.11.5 時 body 沒有 overflow，超出的部分是被卡片的 `overflow:hidden` **靜靜裁掉**——所以「升級後多出捲軸」通常不是升級造成的新問題，而是把既有的高度不足從「看不見的裁切」變成「看得見的捲軸」。已踩過一次：`PD03.vue` 的 `.card` 寫死 `height:110px`，但成員頁多一列 `el-progress`，內容實高 131px，進度條底部一直被切掉約 9px（2026-07-30 改為 `min-height:110px`，卡片自己長到 115.7px）。偵測全頁有無此問題：`document.querySelectorAll('.el-card')` 逐一比較 `body.scrollHeight > body.clientHeight`
- 承上，**查這類版面問題不能只靠「開頁面看一看」**——條件渲染的元件不會出現在當下的 DOM 裡，掃到 0 個問題可能只代表沒渲染出來。可靠做法是**先靜態列出母體，再確認每個都真的被渲染過**：(1) grep 所有 `<el-card>`、抓出各自的 class 與該 class 的高度宣告，分成「寫死 `height`（有風險）／`min-height` 或未宣告（結構上不可能溢出，body 會自己長高）／註解掉的死碼」三類；(2) 只需實測第一類，但要把**出現過的 `cssHeight` 清單當覆蓋率指標**——靜態分析說有 100px 和 220px 兩種卡，實測若沒量到這兩個數字，就代表還沒真正走到那段畫面。2026-07-30 全面稽核 26 個 `el-card` 實例即用此法：2 個是 `PR01.vue` 註解掉的死碼、13 個未寫死高度、11 個寫死高度全部實測完畢。其中兩個特別難觸發——`PD04.vue:714` 的 `.task-card`（100px）要點「編輯工作」開對話框再切工作類別分頁、`PD05.vue:3053` 的 `.card`（220px）要選側欄類別**並且**切到有月份項目資料的月份，前幾輪掃描都沒叫出來，當時的「0 個問題」對它們並不成立
- 版面元素**沒有 margin 時會比有 `.ma8` 的兄弟元素往左凸 8px**：卡片群普遍掛 `.ma8`，底下若接一個裸 `<div>`（如 PD03 成員頁的「合計: N %」），左緣會對不齊、上方也擠在一起。補 `.ma8` 可同時解決間距與左緣對齊；`.ma16` 間距更大但左緣反而往右凸 8px，`.mv8` 只加垂直間距、左緣仍不對齊
...
| P2-13 | 宣告版本對齊實裝版本 ＋ 補 `@element-plus/icons-vue` 宣告 | ✅ | 2026-07-30 | P2-10 重裝後，26 個直接相依的實裝版本超出 `package.json` 寫的下限（`element-plus` ^2.11.5→實裝 2.14.3、`vue` ^3.5.13→3.5.40、`axios` ^1.12.2→1.18.1、`webpack` ^5.102.1→5.109.2…），因為 `npm install` 不改寫範圍（見開發注意事項 (d)）。已從 lock 讀實裝版本回填三個 `package.json`，**只對齊不升級**、前綴 `^` 原樣保留。同時補上 `@element-plus/icons-vue: ^2.3.2`——`src/index.js:11` 有 import 卻從未宣告，是靠 element-plus 的相依被 hoist 才撈得到的 phantom dependency。**驗證**：`npm install` 回報 up to date（零下載）、lock 差異只有宣告區塊無任何 `version` 欄位變動、重 build 產出與回填前位元組完全相同（54 檔／18,238,327 bytes）、`npm ci --dry-run` 通過（P2-10 遺留的 package.json 與 lock 不同步問題一併解決） |
| **P3 使用者可見的改善（成本低）** |||||
...
13. 不通過則退回，**退路分兩層**：
    - **第一層** `git checkout package-lock.json package.json client/package.json server/package.json` ＋ `npm ci`。靠的是版控裡那份 lock 記著上一個確定能跑的完整擺放位置，會原樣重現。**四個檔案必須一起還原**——`npm ci` 會先檢查 `package.json` 與 lock 是否同步，只還原 lock 會因為宣告對不上而直接拒絕執行（2026-07-29 就踩過：P2-11 刪掉 15 個宣告後沒重跑 install，`npm ci` 整個不能用，直到 P2-13 回填才恢復；`npm ci --dry-run` 是不動磁碟就能確認同步狀態的快篩）。失效情境：套件被下架／撤版、斷網、原生模組要重抓 prebuilt binary。
...
| 事後發現的回歸 | Element Plus 2.11.5→2.14.3 改寫 `el-card` 版面，2026-07-30 陸續發現兩處（均已修，見開發注意事項的兩條 `el-card` 條目）：(1) PM02 六個 option-card 文字不再垂直置中；(2) PD03 成員頁卡片冒出捲軸——後者其實是既有的高度不足被 `overflow:auto` 顯影，不是新問題。已逐頁掃過 PD04／PD05／project／overview／document／PM03～05，其餘卡片皆正常。**教訓：驗證步驟 10 的「瀏覽器實跑」只確認了頁面能開、功能能動，沒有逐頁比對版面細節；跨版本升級的 CSS 回歸靠肉眼掃視會漏，日後大版本升級應對重點頁面留 before／after 截圖比對，並跑一次「全頁掃描 `el-card` 是否 `scrollHeight > clientHeight`」這類量化檢查** |
