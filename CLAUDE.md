...
- `PM01.vue`、`service/util.service.js`、`util/utils.js` 使用 `_`（lodash）但未 import，靠相依套件洩漏的全域變數運作——修改這些檔案時應順手補上 `import _ from 'lodash'`
...
3. **便宜除雷**：lodash 補 import（見開發注意事項）；移除未使用的 vuex（含 webpack alias）與 typescript/ts-loader/tsconfig.json；webpack output 加 `[contenthash]`（目前固定 bundle.js，部署後瀏覽器吃舊快取）
4. **排版美工 CP 值最高三項**：導覽列加 `router-link-active` 高亮目前分區；PM02 option-card 顏色改由 `route.path` 推導（棄手動塗色，`sel.hist.link` 可省）；重複的工具 CSS（.ma8/.ma2/.shadow 等）集中至 `assets/style.css`
