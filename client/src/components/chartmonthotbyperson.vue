...
const renderChart = (data, otlist) => {    
    let rdata = [...otlist];
    
    //personInfo.value = `${data.teamno} ${data.employeeno} ${data.name}`;
    pInfo.value = data;
    ...
    let options = {
        title: {
            text: `${pInfo.value.teamno} ${pInfo.value.employeeno} ${pInfo.value.name} 每月加班時數`,
    ...
    myChart.resize({
        width: yearmonth.length * 80 > 300 ? yearmonth.length * 80 : 300,
        height: 250
    });
...
const onClick = (params) => {
    ...
    emits('onClick', {
        teamno: pInfo.value.teamno,
        employeeno: pInfo.value.employeeno,
        name: pInfo.value.name,
        yearmonth: params.name
    });
}
